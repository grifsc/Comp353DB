...
$memberQuery = "SELECT * FROM kqc353_4.FamilyMember WHERE FamilyMemberID = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $query = "UPDATE kqc353_4.FamilyMember SET
            FirstName = ?,
            LastName = ?,
            BirthDate = ?,
            SIN = ?,
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                if (strpos($stmt->error, 'SIN') !== false) {
                    ...
                    $errors['database'] = "Error updating family member: " . $stmt->error;
                }
            }
...
<main>
    <h2>Edit Family Member (ID: <?= htmlspecialchars($familyMemberID) ?>)</h2>
    
    <?php if (!empty($errors['database'])): ?>
        <div class="error-message"><?= htmlspecialchars($errors['database']) ?></div>
    <?php endif; ?>
    
    <form method="post" action="edit.php?FamilyMemberID=<?= $familyMemberID ?>">
        ...
        <div class="form-actions">
            <button type="submit" class="submit-button">Update Family Member</button>
